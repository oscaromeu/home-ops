...
convert_file () {
    ...
    if [[ "$input" == "$output" ]]; then
        echo "Skipping (already mp4): $input"
        return
    fi
...
if [ "$RECURSIVE" = true ]; then
    ...
    for ext in mov mkv avi webm flv wmv ts m4v; do
